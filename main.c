...
ArrayList testWithPrint(ArrayList printArr) {
    ...
    lq
    ->Print(lq, "->", lambda(char *, (int idx, void *item) {
                return (char *)item; })
           );
}
...
ArrayList testWithPrintln(ArrayList printlnArr) {
    ...
    lq
    ->Println(lq, lambda(char *, (int idx, void *item) {
...
ArrayList testWithPrintln3() {
    Linq *lq = Range(0, 20);
    lq
    ->Where(lq, lambda(bool, (void *item) { return TOINT(item) % 3 == 0; }))
    ->Select(lq, lambda(void *, (void *item) {
                    int x = TOINT(item) * 10;
                    return newInt(x);
                }))
